...
from src.core.perform_ops.model import MetricPoint, NetworkMetrics, ResourceMetrics
...
# Istio 메트릭 (HTTP 헤더 → 레이블: x-app-deployment-id)
_TRAFFIC_RPS_QUERY = (
    'sum(rate(istio_requests_total{{x-app-deployment-id="{id}"}}[1m]))'
)
_TRAFFIC_BY_CODE_QUERY = (
    'sum by (response_code)'
    '(rate(istio_requests_total{{x-app-deployment-id="{id}"}}[1m]))'
)
_LATENCY_P95_QUERY = (
    'histogram_quantile(0.95,'
    ' sum by (le)'
    ' (rate(istio_request_duration_milliseconds_bucket{{x-app-deployment-id="{id}"}}[1m])))'
)

# cAdvisor 컨테이너 메트릭 (레이블: x_app_deployment_id)
_CPU_QUERY = (
...
class PrometheusPerformOpsMonitoring(PerformOpsMonitoringClient):
